	...
	//				const Scalar maxRadius = INV_PI * sqrt( nmaxphotons * maxPower / 0.05 );
		...
		protected:
				...
				) const
			{
					...
					distanceToPhoton < nearest.distance )
				{
					...
					const Vector3 vPhotonDir = PhotonDir(this->vphotons[median].theta,this->vphotons[median].phi);
		...
		protected:	
				...
				) const
			{
				...
				LocatePhotons( point, this->dGatherRadius, this->nMaxPhotonsOnGather, heap, 0, this->vphotons.size()-1 );
				...
				) const
			{
				...
				// locate the nearest photons
				PhotonDistListType heap;
				LocatePhotons( ri.ptIntersection, this->dGatherRadius, this->nMaxPhotonsOnGather, heap, 0, this->vphotons.size()-1 );
