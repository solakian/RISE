...
namespace RISE
{
	...
	public:
		...
		void DumpStatistics( const LOG_ENUM e, const unsigned int p ) const
		{
			...
			GlobalLog()->Print( e, p, "BSPTree output: " );
			
			// Call the root node to dump, do this recursively
			unsigned int total_elems = 0;
			unsigned int total_nodes = root.DumpStatistics( e, p, maxPerNode, total_elems );
			GlobalLog()->PrintEx( e, p, "Total nodes in tree: %u", total_nodes );
			GlobalLog()->PrintEx( e, p, "Total elements in all leaf nodes: %u", total_elems );
		}
